...
首先把动力/AI 集合成员相对于当地、当季气候五分位阈值转为每个阈值的 CDF 概率。Debias++ 估计按地区、历日和分位阈值变化的概率偏差，使用过去 20 年同季节历史与最近 3 年选择时窗。Persistence++ 将原集合 CDF、最近已验证观测及 20 年概率气候态做逐格回归，抓住近期持续信号。两路输出再经等距回归投影成单调有效 CDF，最后等权平均。逐次训练只接触目标日期前已完全可观测的数据，以模拟业务发布。[v3 图 1、Methods 4.3](https://arxiv.org/html/2604.16238v3)
...
真值为 ERA5，温度/降水只评分陆地占比 ≥50% 的格点；降水气候分位上界为零的干旱格点排除。概率评价是相对于气候态的 RPSS；**RPSS>0 才优于气候态**，所以低正技巧的“翻倍”与绝对可预测性增强要分开。极端事件改用 5% 分位阈值和 Brier skill score（BSS）。动力基线包含 ECMWF 原始集合与其业务 hindcast 订正版本，AI 基线包括 AIFS-SUBS 和已由 AI 订正 ECMWF 的 PoET；部分 FuXi-S2S 比较限定 2017–2021 且发行日历并不完全一致。[v3 Methods 4.1–4.2](https://arxiv.org/html/2604.16238v3)

历史动力评测覆盖 2016–2024；AIFS-SUBS 结果针对 2025；PoET 对比是 2024。不能把这些不同年份的 RPSS 直接做跨模型统一排名。AI Weather Quest 的 2025 秋季榜单是实时提交，代表较强外部时间验证，但仍需以竞赛协议、目标和评分格点为界。[v3 Results 2.2–2.4](https://arxiv.org/html/2604.16238v3)
