...
> Fabian Mockert、Christian M. Grams、Sebastian Lerch、Julian Quinting，*Windows of opportunity in subseasonal weather regime forecasting: A statistical-dynamical approach*，[arXiv:2505.02680v1](https://arxiv.org/html/2505.02680v1)，2025-05-05 首发。阅读及原文复核：2026-09-25。本文属**次季节预报**，不是 10–15 天逐格天气基础模型。
...
天气型诊断区为 30–90°N、80°W–40°E。作者对 1979–2019 年 6 小时 Z500 做相对 91 天滑动季节气候态的异常、季节标准化和 20 天滤波宽度（约 ±10 天）的低通；取前七个 EOF 后 `k=7` 聚类成七个原型，再投影、归一化为七维天气型指数 IWR。再预报与 ERA5 分别用模式自身 1999–2015、ERA5 1979–2019 的 91 天气候态订正，避免把模式 Z500 偏差误作天气型活动。气旋性型为 Atlantic Trough（AT）、Zonal Regime（ZO）、Scandinavian Trough（ScTr）；反气旋性型为 Atlantic Ridge（AR）、European Blocking（EuBL）、Scandinavian Blocking（ScBL）、Greenland Blocking（GL）。ZO/GL 近似 NAO 正/负位相，但七型不是一维 NAO 的另一种写法。[方法§2.2](https://arxiv.org/html/2505.02680v1)

目标量有三个，不能混为一谈。`WRact_mean` 是先求一周七天 IWR 均值，再以大于 1 记该型活动；ERA5/单成员是 0 或 1，11 成员平均形成以 1/11 为步长的事件概率。`WRact_agg` 是七天中逐日 IWR 大于 1 的**日数占比**，ERA5/单成员以 1/7 递增，11 成员平均以 1/77 递增。七型可同时活跃；`WRact_max` 再从七型中选最大 `WRact_agg` 作唯一主导型，并列时取持续最长者。后文 +2.9 个百分点指这个七分类，不是逐格天气场。[方法§2.3、图 1](https://arxiv.org/html/2505.02680v1)
...
每个天气型/lead 单独训练一个全连接模型：两隐藏层 **64、16**，ReLU，层间 dropout **0.2**，sigmoid 单输出 `[0,1]` 活动度。最多 **50 epochs**、验证 early-stop patience **10**、初始学习率 **0.001**，5 个 epoch 无改善则×0.5，最低 **1e-6**，batch **32**。冬季 1720 样本分成四个连续块，每折 1290 训练/430 测试（1290 中还要分出内部验证），训练内部不打乱取 20% 验证；10 个独立随机初始化网络的输出取均值。作者未在该节明确给出优化器、权重衰减与最终总参数量，不能借常见默认值臆填。[方法§2.7、表 3](https://arxiv.org/html/2505.02680v1)
...
逐步特征选择先单变量训练，再逐项加入使交叉验证 MSE 最低的变量，不同天气型/lead 单独执行。格陵兰阻塞 `NN_all` 先选 ECMWF 第 3 周 IWR 均值及从 IWR 派生的周活动，随后有起报 NAO、GL 季节气候态、年内日期和 SPV/QBO 等；虽然 MJO 条件分析显著，最终 GL 特征选择**没有保留 MJO 特征**。这说明统计“机会窗”不自动意味着在已有 NWP 信息时仍有独立增益。其他型可纳入 MJO/AAO/ENSO 等，图 6 的七型特征序列并不相同；周 2 趋势或相邻起报变化等直觉特征有时很靠后、有时被排除。[方法§2.7、结果§3.3、图 6/S4](https://arxiv.org/html/2505.02680v1)
...
| 图 4 | MJO 7/8/1、弱 SPV 为**气候**高发生率窗口。MJO **8/1** 的 BR′、HR′、PSS′同为正，为 Type 2，但三项都显著仅位相 **1**。MJO **4** 在 GL 较少时 HR/PSS 却较高，为 Type 3。**弱 SPV** 虽 BR/HR 升，FAR 也升、`PSS′<0`，是 Type 1；强 SPV 无有利模式窗口。 | 把弱 SPV 的命中率改善说成 ECMWF 有更高辨识技巧。 |
| 图 5 | 无条件 GL `WRact_agg` 在冬季约 **15%–18%**。MJO 8 条件曲线约从 11 月 **11%** 升至 2 月 **30%**；MJO 1 从 15% 升至约 22% 后约维持 20%；弱 SPV 在 1/3 月峰值约 **24%**，强 SPV 在 1 月低至约 **7%**。 | 把群体条件发生率当个例校准概率，或混同图 3 的周均二值事件。 |
| §3.2 事件合成 / 补图 S2–S3 | 对真实 GL 起始对齐后，NWP 信号一般最强；MJO 条件气候态在起始前几天转为高于无条件基线。固定约 18 天 lead 的 NWP 信号出现约两天起始延迟，SPV 信号较弥散。 | 合成只选**真实发生的 GL**，没有虚警案例；观测/预报采用不同纵轴，不能从合成图推断全样本技巧。 |
| 图 6 / 补图 S4 | GL 最重要为 NWP 的周 3 IWR/活动及起报 NAO；最终 GL `NN_all` 未选 MJO。七型特征排序不同。 | 把特征入选当因果作用，或声称 MJO 普遍主导所有天气型。 |
| 图 7 | `MSESS_NWP=1−MSE_model/MSE_NWP`。`NN_all` 七型活动 MSE 均优于原始 NWP，改善 **3.0%–9.7%**，ScBL 最大。原 NWP 对 EuBL/ScBL 难胜普通气候态，`NN_noNWP` 在两者上也胜 NWP；`NN_NWP+WR` 增益较小。对本来较有技巧的 GL，只有 `NN_all` 继续改善。 | 将最大增益说成所有型/所有变量的改善；忽视无 NWP 版本使用了更长训练期。 |
| 图 8 | 主导型七分类 `WRact_max` 总准确率 **28.7%→31.6%**，即 **+2.9 个百分点**、约 **+10% 相对改善**。除 ScTr 外各型 HR 上升；FAR 在 ZO/ScTr/EuBL/ScBL 降，GL 持平，AT/AR 反而升。 | 写成“31.6 个百分点增益”，或声称每型所有指标和逐格温度/降水预报都提高。 |

这些约数按[原文§3 与图 1–8](https://arxiv.org/html/2505.02680v1)及作者正文说明转录，不从像素估读捏造小数。尤其要同时读图 3–5 与图 7–8：前者描述**何时较可能发生/较易分辨**，后者评估**融合后的实际误差**，目标量和分母均不同。`NN_noNWP` 对部分阻塞型有效很有启发，但训练资料比依赖 ECMWF 回报的网络更长，不能把网络间差异纯粹解释为某一输入源的因果效应。
