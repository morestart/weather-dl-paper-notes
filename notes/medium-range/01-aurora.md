...
| 热带气旋路径 | 至 5 天；0.25° 天气分支的确定性输出经最低海平面气压追踪 | 对四海盆中所列机构、可配对时效的路径 MAE 更低；北大西洋/东太平洋第 1 天约好 **6%**，第 2–5 天约好 **20%–25%** | **不针对台风单独微调**；机构并非每次都发布相同预报，不能把图 4 的不同列当成同样本排行榜 |
...
### 4.2 正式主图逐幅阅读：什么是同一模型，什么不是同一评测

| 图与面板 | 原文具体展示 | 该图不能证明什么 |
| --- | --- | --- |
| **图 1a–b** | 多来源预训练→空气质量/海浪/气旋路径/0.1° 天气的任务适配，以及 Perceiver 编码器—3D Swin 主干—Perceiver 解码器。 | 结构图只是网络与迁移路线，不给任意变量或任意分辨率的零样本技巧分数。 |
| **图 2a–b：CAMS** | 五日空气质量：全目标约 **74%** 匹配或优于 CAMS，三日截面约 **89%** 变量达到此标准；另有 **95%** 全目标落在 CAMS RMSE 的 20% 范围内。 | “接近 20%”与“胜出”是两个阈值；不能并成天气任务的 95% 优胜率。低层最初 12 小时和高层臭氧仍有 CAMS 优势。 |
| **图 3a–c：波浪** | 图 3a 以 2022-09-16 12 UTC 起报、次日 12 UTC 的南玛都波高/方向个例；全时效 **86%** 波浪目标不差于 HRES-WAM，三日地表波浪变量约 **91%**。 | 例图是一天预测，不是十天灾害波高个例；峰值与方向图也不能代替全域概率校准。三日 PP1D 仍未达到“RMSE 差距≤20%”的接近阈值。 |
| **图 4a–b：气旋** | 2022–2023 全球四海盆，以 0.25° **HRES T0 天气微调版**的逐时 MSL 最小值追踪涡旋中心，对 IBTrACS 路径计算 MAE；图 4a 报告北大西洋/东太平洋平均优于机构约 **20%**、西北太平洋约 **18%**、澳大利亚海区约 **24%**。图 4b 的杜苏芮是提前四天判断菲律宾登陆、机构路径偏向台湾附近的一个正例。 | 这些是路径不是强度技巧。图注明确不同机构/模式并非每场都出报，**每列样本不同**；`≈` 表示该格 95% 区间含零。因此“所有机构和时效平均更优”不代表每个格都统计显著，更不能对列间数值作公平排名。 |
| **图 5a–d：0.1° 天气** | 图 5a 仅下层可评层次的变量×层×时效 RMSE scorecard；图 5b 是 13,000 多站的 **10m 风速 RMSE** 曲线，正文又称站点 T2m/风至十天更优。图 5c 为 0.1° HRES 分析对照的 Ciarán 路径与最大 10m 风；图 5d 比较风暴最大 10m 风速突增，其他 AI 的数字取自另一篇研究的图 3。 | `92%` 不是对全高空或所有站点的胜率。图 5c–d 的 Aurora **关闭 LoRA**，与主 scorecard 配置不同；5d 的其它 AI 数字是跨论文借用，需核实其初值/真值/版别才可当严格头对头。单次风暴也不能推成全部极端风事件。 |

图 4 的台风证据尤须分清**权重训练**与**诊断后处理**：作者没有针对台风重训 Aurora，而是在已微调的 0.25° HRES T0 天气场中，用连续时次最低海平面气压提取中心，再与官方机构发布的路径按可配对样本比较；图 4 的机构/海盆平均改善不能归功于“台风专用 loss”。图 5 的 0.1° 实验又是另一条任务分支，不能把 0.25° 台风路径均值并入 0.1° 网格天气的 92% 分母。[正式正文“Atmospheric chemistry”“Ocean wave dynamics”“Tropical cyclone tracking”“High-resolution weather forecasting”及图 2–5 图注](https://www.nature.com/articles/s41586-025-09005-y)
